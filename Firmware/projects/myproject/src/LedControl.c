...
void sendByte(const uint8_t reg, const uint8_t data)
{
	...
	buffer = &aux;
	Chip_SSP_WriteFrames_Blocking(LPC_SSP1, buffer, 1);	// Envio por SPP1
...
void msjScroll()
{
	...
	switch(mode)
	{
		...
		case 2:
		{
			...
			printStringScroll(1, mensaje_actual, velocidad, '<');
			LCD_delay_ms(250);
			clearDisplay();     //borrar pantalla '0'
			break;
		}
